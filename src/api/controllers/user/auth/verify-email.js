...
export default async (req, res) => {
  ...
  const existOtp = await Otp.findOneAndDelete({
    userId: req.user._id,
    code: req.body.code,
  }).catch((err) => {
    return res.status(500).json(errorHelper("00051", req, err.message));
  ...
  if (!existOtp) {
    const otpData = await Otp.findOne({ userId: req.user._id });
    if (otpData.countVerify <= 5) {
      await Otp.updateOne(
        { userId: req.user._id },
        { $set: { countVerify: otpData.countVerify + 1 } }
      ).catch((err) => {
        return res.status(500).json(errorHelper("00056", req, err.message));
      });
      return res.status(400).json(errorHelper("00054", req));
    }
    if(!otpData.isVerifyOver){
      await Otp.updateOne(
        { userId: req.user._id },
        { $set: { isVerifyOver: true } }
      ).catch((err) => {
        return res.status(500).json(errorHelper("00056", req, err.message));
      });
    }
    
    return res.status(400).json(errorHelper("00094", req));
  }
